...
{   
    ...
    //float3 diffuse  = diff * diffValue;
   // float3 specular = spec * diffValue;
    
    float distance    = length(poiLight.position - gBuffer.WorldPos);
    float attenuation = 1.0f / (1 + 1.5f * distance + 1.5 * (distance * distance));
    float3 diffuse  =  attenuation * float3(1.0f, 0.0f, 0.0f);
    float3 specular =  attenuation * float3(1.0f, 0.0f, 0.0f);
